...
26 annotations in `permuplate-annotations/`. For per-annotation API detail and usage examples, see [OVERVIEW.md § Annotation API Detail](OVERVIEW.md#annotation-api-detail).
...
| `@PermuteNew` | (TYPE_USE on `new` expr) | Explicit constructor type rename: evaluates `className` JEXL and renames the `new X<>()` type. Complements constructor-coherence inference for cases with multiple generated-class constructors in one body. Maven plugin only. |
...
| `inline=true` on top-level class templates | Supported since the fix to `InlineGenerator.generate()`. When the template is not nested, generated classes are added to `outputCu.getTypes()` rather than `outputParent.getMembers()`. The template class name must differ from all generated class names (use a distinct template name like `RuleBuilderTemplate` that generates `RuleBuilder`; or for non-template mixin classes use `@PermuteMixin` without `@Permute` — see `@PermuteMixin` on non-template classes in the decisions table). This enables `@PermuteMethod` on top-level classes. |
...
| `@PermuteNew` inside `@PermuteMethod` limitation | The `className` expression is evaluated with the outer loop variable (`i`) in scope but NOT the inner loop variable (`j`). Use `@PermuteReturn` instead for methods where the constructor type expression must reference the inner variable. |
