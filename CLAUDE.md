...
| Two independent `@PermuteTypeParam` axes fail in cross-product templates | Templates using `@PermuteVar` (cross-product) with two `@PermuteTypeParam` annotations — one per axis variable — produce duplicate type parameters in generated output. Single-axis expansion works correctly. Cross-product templates must retain `Object` types for both parameter groups until this limitation is resolved. |
