...
| `typeArgList` in `@PermuteDeclr.type` — use single quotes | Inside `@PermuteDeclr(type = "Callable${i}<${typeArgList(1,i,'alpha')}>")`, use single quotes for the style argument. Both quote styles work in JEXL3, but single quotes avoid backslash noise in Java source. |
| Two independent `@PermuteTypeParam` axes in cross-product templates | `expandExplicit` processes each annotated type parameter in sequence, building a single result list — both axes expand cleanly with no duplicates. `expandImplicit` is bypassed for both sentinels because `alreadyExpanded` contains them after the explicit pass. |
| `@PermuteTypeParam` on non-first type param — fixed A plus expanding tail | Placing `@PermuteTypeParam(from="2", to="${i}")` on `B` in `Join2<A, B>` keeps `A` fixed and expands only `B` onwards: `Join3<A, B, C>`, `Join4<A, B, C, D>`. Use `from="2"` so alpha naming continues naturally from `B`. The standard generic template pattern for Join/Callable families. |
...
| `transformNewExpressions` signature | Now `public static void transformNewExpressions(Node scope, EvaluationContext ctx)` — takes any `Node` (not just `TypeDeclaration<?>`) so it can be called on a cloned method inside `applyPermuteMethod` with the inner (i,j) context. |
