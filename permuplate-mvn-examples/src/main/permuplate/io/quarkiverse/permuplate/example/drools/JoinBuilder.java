...
/**
 * Container for the JoinFirst and JoinGate class families generated by Permuplate.
 *
 * <p>Two template classes are declared here:
 * <ol>
 *   <li>{@code Join0Gate} — generates {@code Join1Gate..Join6Gate}: the "gateway"
 ...
 *       family with {@code filter()} and {@code fn()}, extending the corresponding Gate.</li>
 * </ol>
 *
 * <p><b>Declaration order matters:</b> {@code Join0Gate} is declared before {@code Join0First}
 ...
 *
 * <p><b>Bi-linear joins:</b> {@code join(JoinNGate)} overloads on {@code Join0Gate}
...
public class JoinBuilder {
    ...
    /**
     * Template generating Join1Gate through Join6Gate.
     *
     ...
     *   <li>Bi-linear: {@code join(JoinNGate<Void,DS,...>)} — generated by
     *       {@code @PermuteMethod(j)} for all valid right-chain arities. The pre-built
     *       chain always uses {@code Void} as END (no outer scope).</li>
     * </ul>
     */
    @PermuteSealedFamily(interfaceName = "JoinBuilderGate", typeParams = "END, DS")
    @Permute(varName = "i", from = "1", to = "6", className = "Join${i}Gate",
             inline = true, keepTemplate = false)
    public static non-sealed class Join0Gate<END, DS,
            @PermuteTypeParam(varName = "k", from = "1", to = "${i}", name = "${alpha(k)}") A>
            extends BaseRuleBuilder<END>
            implements JoinGate<DS> {

        protected final RuleDefinition<DS> rd;

        public Join0Gate(END end, RuleDefinition<DS> rd) {
            super(end);
            this.rd = rd;
        }
         ...
         *
         * <p>Boundary omission removes this from Join6Gate — Join7First is not in the
         ...
         *
         * <p>The right chain (a pre-built {@code JoinNGate} or {@code JoinNFirst} since
         * First extends Gate) executes independently against the same ctx. Its matched
         ...
         * is why the parameter type is {@code JoinNGate<Void, DS, ...>} — the chain was
         ...
         * {@code @PermuteDeclr} sets the parameter type to the correct {@code JoinNGate} type.
         */
        ...
        public <@PermuteTypeParam(varName = "k", from = "${i+1}", to = "${i+j}",
                ...
                @PermuteDeclr(type = "Join${j}Gate<Void, DS, ${joinRight}>")
                Object secondChain) {
            @SuppressWarnings("unchecked")
            JoinGate<DS> second = (JoinGate<DS>) secondChain;
        ...
        @PermuteReturn(className = "${Scope}Scope",
                       typeArgs = "'Join' + i + 'Gate<END, DS, ' + alphaList + '>, DS'",
                       alwaysEmit = true)
         ...
         * Suppressed on Join6Gate ({@code when="i < 6"}) — no Join7First exists.
         *
         ...
         *
         * <p>Placed on Join0Gate so that after {@code not()...end()} or
         * {@code exists()...end()} (which return JoinNGate), the fluent chain
     ...
     *
     * <p>Extends {@code Join0Gate<END, DS, A>} — G3 extends clause auto-expansion
     * detects "Join" prefix + embedded number 0 + alpha prefix match, expanding to
     * {@code Join1Gate<END, DS, A>}, {@code Join2Gate<END, DS, A, B>}, etc.
     *
     * <p>Holds {@code filter()} (single-fact and all-facts overloads). All inherited from
     * the template; inherited {@code join()} and {@code fn()} come from Gate via extends.
     */
    ...
    public static non-sealed class Join0First<END, DS,
        ...
        /**
         * Cross-fact filter using m bound variables (m=2..6).
         * All variables must have been bound via var() before this call.
         * Generated by @PermuteMethod for m=2..6; delegates to addVariableFilter()
         * on RuleDefinition which is itself generated by VariableFilterMixin.
         */
