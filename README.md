...
| `macros` | `String[]` | Named JEXL expression macros evaluated per-permutation after all loop variables are bound. Format: `"name=jexlExpression"`. The result is stored as `${name}` and is available in any JEXL expression throughout the template. Later macros may reference earlier ones (evaluated in declaration order). Example: `macros = {"doubled=i*2"}` makes `${doubled}` expand to `4` when `i=2`. Malformed entries (no `=`) are silently skipped. |
